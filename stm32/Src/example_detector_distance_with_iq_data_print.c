...
static void set_config(acc_detector_distance_config_t *detector_config,
	...
	case DISTANCE_PRESET_CONFIG_BALANCED:
		...
		*/
	    ...
	    acc_detector_distance_config_sensor_set(detector_config, SENSOR_ID);

	    acc_detector_distance_config_start_set(detector_config, 0.050f);
	    acc_detector_distance_config_end_set(detector_config, 2.400f);
	    acc_detector_distance_config_max_step_length_set(detector_config, 0U);

	    acc_detector_distance_config_signal_quality_set(detector_config, 35.000f);
	    ...
	    acc_detector_distance_config_threshold_sensitivity_set(detector_config, 0.500f);
	...
	case DISTANCE_PRESET_CONFIG_HIGH_ACCURACY:
	    ...
	    acc_detector_distance_config_sensor_set(detector_config, SENSOR_ID);
		acc_detector_distance_config_start_set(detector_config, 0.08f);
		acc_detector_distance_config_end_set(detector_config, 02.4f);
		acc_detector_distance_config_max_step_length_set(detector_config, 0U); //was 2
	...
	//	acc_detector_distance_config_peak_sorting_set(detector_config,	ACC_DETECTOR_DISTANCE_PEAK_SORTING_STRONGEST);
		...
		acc_detector_distance_config_threshold_method_set(detector_config, ACC_DETECTOR_DISTANCE_THRESHOLD_METHOD_FIXED_AMPLITUDE);
		acc_detector_distance_config_fixed_amplitude_threshold_value_set (detector_config, 40.0f);


//		acc_detector_distance_config_threshold_method_set(detector_config, ACC_DETECTOR_DISTANCE_THRESHOLD_METHOD_CFAR);


//		acc_detector_distance_config_threshold_method_set(detector_config, ACC_DETECTOR_DISTANCE_THRESHOLD_METHOD_RECORDED);
		acc_detector_distance_config_threshold_sensitivity_set(detector_config,	0.8f);

//		acc_detector_distance_config_receiver_gain(detector_config,	0.5f);

		acc_detector_distance_config_num_frames_recorded_threshold_set(	detector_config, 0U); // was 100

		acc_detector_distance_config_signal_quality_set(detector_config, 20);
...
static void print_iq_data(const acc_config_t *sensor_config,
	...
	// Create a uart buffer of 64 bytes
	char uart_buffer[5000];
	...
	for (uint8_t subsweep_idx = 0; subsweep_idx < num_subsweeps;
		...
		for (uint16_t point_idx = 0; point_idx < subsweep_length; point_idx++) {
			...
			uart_buffer_index += sprintf(uart_buffer + uart_buffer_index,
					"%.4f,%ld\n", distance, iq_point_abs);
		}
	}

	// Print the uart buffer
...
//	if (result->num_distances > 9) result->num_distances=9;  // jmm
...
//		printf(" \n REJECTED DUE TO OUT OF RANGE WITH RESPECT TO LAST READING");
	...
	// ADDED jan 27 2023 jmm
	float Result_Strength = 100;
	float Strength_Threshold = 50;	//CHECK THE AMPLITUDE / STRENGTH
	if (Result_Strength > Strength_Threshold) {
		printf(" \n REJECTED DUE TO STRENGTH LOWER THEN Strength_Threshold ");
		Current_Shortest_Distance_in_MM = 2000;
	//	Rejection_Flag = true;
	}
*/
...
//	float Mapped_Value = 100- (Current_Shortest_Distance_in_MM / 2400.0) * 100.0;
//	if (50 > Current_Shortest_Distance_in_MM ) Mapped_Value = 00;
